...
const context = { vault: 'sample-vault', publication: 'example-digest' }
test('privacy evidence must cover every surface, identity, configuration and fresh publication', () => {
  ...
  const mutations = [e => { e.vault = 'another-vault' }, e => { e.publication = 'old' }, e => { e.validUntil = 0 }, e => { e.checkedAt += 600000 }, e => { e.configuration.completeInventory = false }, e => { e.targets.pop() }, e => { e.targets[0].owner = 'denied' }, e => { e.targets[0].anonymous = 'error' }, e => { e.targets[0].otherUser = 'redirect' }, e => { e.targets[0].url = 'https://example.test/?credential=redacted' }]
