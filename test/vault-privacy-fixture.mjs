...
export function protectionEvidence(context) {
  const { vault, publication, revision, phase, owner, manifest = [], objects = [], deployment: site = deployment } = context
  ...
  return { vault, publication, revision, phase, owner, deployment: site, checkedAt: Date.now(), validUntil: Date.now() + 60000, policyRevision: 'synthetic-policy', configuration: { ownerOnly: true, privateStorage: true, completeInventory: true }, targets }
}
