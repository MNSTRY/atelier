...
const input = revision => ({ schema: 'atelier-vault-publication/v1', expectedRevision: revision, files: [{ path: 'report.html', base64: Buffer.from(`Revision ${revision}`).toString('base64') }, { path: 'style.css', base64: Buffer.from('body { color: green }').toString('base64') }] })
test('real service and denial collector compose without recursion; activation covers every staged key', async () => {
  let record = { owner, revision: 0, manifest: [], publication: null }, inspections = 0, collections = 0
  const bytes = new Map(), evidenceStore = new Map()
  ...
  const privacy = createVaultPrivacyState({ probe: { async verify(context) { collections++; return probe.verify(context) } }, load: async key => evidenceStore.get(key), save: async (key, evidence) => evidenceStore.set(key, evidence) })
  ...
  const context = { vault: 'sample-vault', owner, deployment, publication: record.publication, manifest: record.manifest, objects: vaultObjects('sample-vault', record.manifest) }
...
test('binding refuses missing origins, requested paths, manifest digests and staged object coverage', async () => {
  ...
  const context = { vault: 'sample-vault', publication: prepared.publication, phase: 'before-activation', owner, deployment, manifest: prepared.manifest, objects: vaultObjects('sample-vault', prepared.manifest) }
  ...
  assert.equal(assessVaultPrivacy(protectionEvidence({ ...context, phase: 'read' }), { ...context, phase: 'read', request: { origin: deployment.origins[0], path: '/sample-vault/missing.html' } }).status, 'unknown')
})
test('exposure survives stale metadata and failed notification; cached alarm cannot be cleared by refresh', async () => {
  let calls = 0, notifications = 0, inspections = 0, stored
  const canary = 'synthetic-canary-marker-01234567890123456789'
  const context = { vault: 'sample-vault', publication: 'synthetic', phase: 'before-upload', owner, deployment, manifest: [], objects: [] }
  const probe = createVaultPrivacyProbe({ inspect: async () => { inspections++; if (inspections > 1) return new Promise(() => {}); return { ...protectionEvidence(context), targets: [{ kind: 'route', url: 'https://artifacts.example/sample-vault', sha256: '0'.repeat(64), canary }] } }, request: async () => { calls++; return new Response(`prefix ${canary} suffix`, { status: 403 }) }, onExposure: () => { notifications++; throw new Error('notification unavailable') } })
  const state = createVaultPrivacyState({ probe, load: async () => stored, save: async (_, value) => { stored = value } })
