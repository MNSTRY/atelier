...
test('full observation proves a normal local clone complete', (t) => {
  ...
  runGit(git, root, ['config', 'filter.lfs.clean', 'git-lfs clean -- %f'])
  runGit(git, root, ['config', 'filter.lfs.smudge', 'git-lfs smudge -- %f'])
  runGit(git, root, ['config', 'filter.lfs.process', 'git-lfs filter-process'])
  ...
  assert.deepEqual(report.features.customFilters, [])
