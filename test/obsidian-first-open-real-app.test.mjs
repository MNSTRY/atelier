...
// Obsidian, in the two states of the app a first open meets on a desktop. It
// opens application windows, so it never runs by default:
//
...
test('real isolated Obsidian: open adds the view\'s vault and publishes, with the app running on another vault and with the app quit, and is current with no step by hand',
  { skip: !ENABLED && 'set ATELIER_OBSIDIAN_FIRST_OPEN=1 on a macOS desktop with Obsidian installed; opens application windows', timeout: 900000 }, async (t) => {
    ...
    await t.test('app running with an unrelated vault: the vault is added through the app, which opens it, and the first publication runs through the app', async () => {
      ...
      try {
        ...
        assert.deepEqual(await world.appProbe.vaultState({ vaultRoot: store.vaultRoot }), { answered: true, indexReady: true })
