...
const domId = (model, kind, id) => model.id + ':' + kind + ':' + id
...
export function renderPresentation(model, { tokenOverrides = {}, interactive = true } = {}) {
  ...
  const renderAction = node => {
    ...
    return '<div class="ap-action"><button type="button" id="' + name + '" data-ap-action="' + node.id + '"' +
  ...
  const renderNode = node => {
    ...
    if (node.items) {
      ...
      body = '<' + tag + '>' + node.items.map((item, index) => '<li' + (interactive && node.reorderable ? ' draggable="true" data-ap-drag="' + node.id + '" data-ap-item="' + item.id + '"' : '') + '>' +
        (!interactive ? '<span>' + escape(item.label) + (item.selected ? ' (selected)' : '') + '</span>' : item.href ? '<a class="ap-link" href="' + escape(item.href) + '">' + escape(item.label) + '</a>' :
          '<button type="button" data-ap-select="' + node.id + '" data-ap-item="' + item.id + '"' +
        ...
        (interactive && node.reorderable ? '<div class="ap-actions"><button type="button" data-ap-move="' + node.id + '" data-ap-item="' + item.id + '" data-ap-position="' + (index - 1) + '"' + (index === 0 ? ' disabled' : '') + ' aria-label="Move ' + escape(item.label) + ' earlier">Move earlier</button><button type="button" data-ap-move="' + node.id + '" data-ap-item="' + item.id + '" data-ap-position="' + (index + 1) + '"' + (index === node.items.length - 1 ? ' disabled' : '') + ' aria-label="Move ' + escape(item.label) + ' later">Move later</button></div>' : '') + '</li>').join('') + '</' + tag + '>'
      if (node.type === 'graph') {
    ...
    if (noticeTypes.has(node.type)) body = '<div class="ap-notice" data-tone="' + node.tone + '"><p' + (node.type === 'status' ? ' role="status"' : '') + '>' + escape(node.text) + '</p><dl>' +
    ...
    if (node.type === 'field' || node.type === 'editor') {
      ...
      const attrs = ' id="' + fieldId + '" data-ap-edit="' + node.id + '"' +
...
export function renderPresentationDocument(model, options = {}) {
  assertPresentation(model)
  return '<!doctype html><html lang="' + model.lang + '" dir="' + model.direction + '"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' + escape(model.title) + '</title></head><body>' + renderPresentation(model, options) + '</body></html>'
}
