...
import { harnessDigest, harnessRef } from '../harnesses/contracts.mjs'
...
export function knowledgeGraphProposal(records, { namespace, activationId, dependencySnapshots = [] }) {
  ...
  for (const record of selected.values()) {
    ...
    if (record.kind === 'relation') {
      const domain = byId.get(d.domain.id)
      edge(d.subject.id, domain.data.vocabulary.relations.find(r => r.id === d.predicate).graphPredicate, d.object.id)
      continue
    }
    for (const pin of d.basedOn) edge(pin.contribution.id, 'evidences', record.id)
    const review = records.findLast(r => r.kind === 'review' && r.data.target.id === record.id)
    const text = JSON.stringify({ contribution: record, record: harnessRef(record), review, evaluations: review.data.evaluations.map(pin => byId.get(pin.id)), activation: harnessRef(activation), semanticTruthVerified: false }, null, 2)
    const fence = '`'.repeat(Math.max(3, ...[...text.matchAll(/`+/g)].map(m => m[0].length + 1)))
    files.push({ path: `${record.id}.md`, content: ['---', `title: ${JSON.stringify(d.title)}`, 'kg:', `  id: ${JSON.stringify(node(record.id))}`, '  type: "document"', '  status: "draft"', `  audience: ${JSON.stringify(d.audience)}`, '  relations: {}', '---', '', 'Proposed knowledge source. Review the destination and admission before applying.', '', `${fence}text`, text, fence, ''].join('\n') })
  }
