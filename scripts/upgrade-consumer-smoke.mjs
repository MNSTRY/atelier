...
function verifyUpgrade({ installedRoot, consumerRoot }) {
  ...
  const explanation = cli('explain', '--plan', plan.savedPlan)
  assert.equal(explanation.status, 0, explanation.stderr)
  const report = JSON.parse(explanation.stdout)
  assert.equal(report.planDigest, plan.plan.digest)
  assert.equal(report.consent.applicationAuthorized, false)
  assert.equal(report.bindingsCurrent, true)
  for (const surface of ['codex', 'claude']) assert.ok(fs.existsSync(path.join(installedRoot, 'skills', surface, 'atelier-guided-upgrade', 'SKILL.md')))
  assert.equal(cli('explain', '--plan', plan.savedPlan, '--format', 'markdown').status, 0)
