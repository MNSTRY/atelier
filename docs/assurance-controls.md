...
| User-confirmed repository writes match what was reviewed | Content-bound plan identifiers, no-follow plan leaves, fresh complete observations, exact staged blob/mode manifests, commit-tree verification with rollback, and publish-target revalidation | `sync:test` plan, hook/index, remote-drift, and evidence-failure cases | Ordinary Git hooks may run, but any tree drift is rolled back and cannot be published |
| Resident Sync state cannot escape or grow without bound | Contained private directories, no-follow/identity-checked leaves, nonce-bound locks with atomic stale recovery, and hash-chained trace checkpointing under byte/record ceilings | `sync:test` redirected-state, concurrent-recovery, corruption, and trace-ceiling cases | Machine-local state is diagnostic convenience; Git and readable repository files remain authoritative |
