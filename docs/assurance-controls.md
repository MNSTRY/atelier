...
| The published artifact is the artifact tested | `release:candidate` packs once, binds SHA-256, and sends that exact archive through allowlist/content audit, bare-consumer install, all-export import, and branded distribution smoke | `release:audit`, `consumer:smoke`, `distribution:smoke`, `assurance:mutation-smoke` | Passing gates does not publish, deploy, or constitute an external reviewer’s approval |
