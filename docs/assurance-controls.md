...
| A clean boundary verdict uses complete evidence | Typed Git reads with diff and binary budgets; incomplete reads become blocking diagnostics; Sync additionally requires the enrolled root to be one of the policy's scanned repositories | `test/boundary-content-rules.test.mjs`; Sync boundary-scope refusal case | Budgets intentionally refuse evidence too large to inspect rather than claiming it is clean |
