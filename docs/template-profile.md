...
Status: Next-release package API. Import validators from
`@mnstry/atelier/templates` and canonical project binding from
`@mnstry/atelier/template-bindings`. The versioned schema is exported at
`@mnstry/atelier/contracts/atelier-template-profile.v1.schema.json`.
These APIs do not establish installed product-host acceptance.
...
`@mnstry/atelier/template-bindings` exports `createTemplateProjectView(project,
...
`npm run consumer:smoke` installs the exact tarball offline and exercises the
template APIs by package name. Its synthetic journey binds canonical source,
checks provider absence and audience refusal, applies an edit through the
installed source-apply owner, rejects the stale binding, and checks idempotence
and preservation of an unrelated source. Atomic exchange is required for the
write portion; other platforms retain the read-only checks.

The built-in [template adoption participant](template-upgrades.md) connects local
profile adoption, exact locks, retained history and read-only projections to the
existing upgrade transaction engine. External packs, runtime extensions and
automatic restoration remain unsupported.

Remaining integration includes interactive host wiring, all-carrier conformance,
actual host/accessibility/visual proof, and receiving-owner acceptance.
