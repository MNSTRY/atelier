...
redacted: generated regions, the identity block, note and attachment paths,
manifest entries and link inversions never carry a withheld node's identity,
title, summary, tags or path. A scoped vault is not a confidentiality boundary
against what visible authors wrote. A redaction boundary over authored bytes
cannot coexist with byte-faithful emission; it would be a separate mode with
its own evidence.
...
guard. The same words trigger it when they reach generated text, for
instance an in-view title that names a withheld identity, because the title is
repeated in the relation rows of other notes. Canonical identities are
matched exactly, so an identity that is an ordinary word refuses a view whose
generated titles use that word as a whole token; Atelier's own identities
are repository-qualified (`<repository>:<name>`).
