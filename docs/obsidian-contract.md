...
The eligibility a view is built with is decided by the machine settings of
the workspace (`eligibilityFor` in `src/runtime/obsidian/pipeline.mjs`), and
the engine, source apply, the proposal adapter and the selection operation
all ask it, so they agree on what a view holds. By default
(`DEFAULT_ELIGIBILITY`, revision
`classified-documents/v2+assets-embedded-by-eligible-documents/v1`) a node is
eligible only when the census classified it. When the audience decision is
`only-you` with `unclassified: shown`, and only then, an unclassified note is
eligible too, provided its bytes pass the byte lens the emitter reads notes
with (`onlyYouEligibility`, revision
`classified-documents/v2+unclassified-notes-read-as-notes-for-only-you/v1+assets-embedded-by-eligible-documents/v1`).
An unclassified note carries the `private` audience, which "only you" admits.
An admitted note is an ordinary member of the view for the redaction guard:
its identity may appear in generated bytes of that view, and stays forbidden
in every view it is not part of. Assets follow the documents that embed them.
